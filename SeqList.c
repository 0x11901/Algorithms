...
int SeqList_Insert(SeqList *list, SeqListNode *node, int pos) {
    ...
    TSeqList *t = list;
    if (t->length >= t->capacity) { return -2; }
    if (pos < 0 || pos >= t->capacity) { return -3; }
    int length = t->length;
    if (pos >= length) { pos = length; }

    for (int i = t->length; i > pos; --i) { t->node[i] = t->node[i - 1]; }
...
SeqListNode *SeqList_Get(SeqList *list, int pos) {
    ...
    TSeqList *t = NULL;
    t = (TSeqList *)list;
    if (list == NULL || pos < 0 || pos >= t->length) { return NULL; }
...
SeqListNode *SeqList_Delete(SeqList *list, int pos) {
    ...
    for (int i = pos + 1; i < t->length; ++i) { t->node[i - 1] = t->node[i]; }
    t->length--;
    return res;
}
